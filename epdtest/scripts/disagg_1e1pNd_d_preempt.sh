...
# Shared 1e1pNd runner with forced decode-over-others preempt bias for both
...
# On GPUs shared with decode, this script always deprioritizes encoder/prefill
# and prioritizes decode.
...
NON_DECODE_NICE="${NON_DECODE_NICE:-10}"
NON_DECODE_MPS_ACTIVE_THREAD_PERCENTAGE="${NON_DECODE_MPS_ACTIVE_THREAD_PERCENTAGE:-20}"
DECODE_MPS_ACTIVE_THREAD_PERCENTAGE="${DECODE_MPS_ACTIVE_THREAD_PERCENTAGE:-100}"
NON_DECODE_CUDA_DEVICE_MAX_CONNECTIONS="${NON_DECODE_CUDA_DEVICE_MAX_CONNECTIONS:-1}"
DECODE_CUDA_DEVICE_MAX_CONNECTIONS="${DECODE_CUDA_DEVICE_MAX_CONNECTIONS:-32}"
...
ENCODER_SHARES_DECODE_GPU=0
PREFILL_SHARES_DECODE_GPU=0
declare -a DECODE_SHARES_OTHER=()
...
parse_decode_ports() {
    ...
    for port in "${DECODE_PORT_LIST[@]}"; do
        ...
        if contains_value "$port" "$ENCODE_PORT" "$PREFILL_PORT" "$PROXY_PORT"; then
            die "Decode port collision detected: ${port}. Choose non-overlapping DECODE_PORT/DECODE_PORTS."
        fi
    done
}
...
configure_decode_preempt_layout() {
    DECODE_SHARES_OTHER=()
    ENCODER_SHARES_DECODE_GPU=0
    PREFILL_SHARES_DECODE_GPU=0
    PREEMPT_SHARED_GPU_CSV=""
    ...
    if contains_value "$GPU_E" "${DECODE_GPUS[@]}"; then
        ENCODER_SHARES_DECODE_GPU=1
        shared_gpus+=("$GPU_E")
    fi

    if contains_value "$GPU_P" "${DECODE_GPUS[@]}"; then
        PREFILL_SHARES_DECODE_GPU=1
        if ! contains_value "$GPU_P" "${shared_gpus[@]}"; then
            shared_gpus+=("$GPU_P")
        fi
    fi

    for gpu in "${DECODE_GPUS[@]}"; do
        if [[ "$gpu" == "$GPU_E" || "$gpu" == "$GPU_P" ]]; then
            DECODE_SHARES_OTHER+=(1)
        else
            DECODE_SHARES_OTHER+=(0)
        fi
    done
...
print_layout() {
    echo "1e1pNd decode-preempt layout"
    ...
    echo "  d_preempt_bias  : always (decode is prioritized on shared GPUs)"
    if [[ -n "$PREEMPT_SHARED_GPU_CSV" ]]; then
        echo "  shared_d_gpus   : $PREEMPT_SHARED_GPU_CSV"
        echo "  non_decode_nice : $NON_DECODE_NICE"
        echo "  non_decode_mps  : $NON_DECODE_MPS_ACTIVE_THREAD_PERCENTAGE"
        echo "  decode_mps      : $DECODE_MPS_ACTIVE_THREAD_PERCENTAGE"
    else
        echo "  shared_d_gpus   : none"
    fi
...
declare -a ENCODE_ENV=("CUDA_VISIBLE_DEVICES=$GPU_E")
if [[ "$ENCODER_SHARES_DECODE_GPU" == "1" ]]; then
    ENCODE_ENV+=(
        "CUDA_MPS_ACTIVE_THREAD_PERCENTAGE=$NON_DECODE_MPS_ACTIVE_THREAD_PERCENTAGE"
        "CUDA_DEVICE_MAX_CONNECTIONS=$NON_DECODE_CUDA_DEVICE_MAX_CONNECTIONS"
    )
fi
...
if [[ "$ENCODER_SHARES_DECODE_GPU" == "1" ]] && command -v nice >/dev/null 2>&1; then
    nice -n "$NON_DECODE_NICE" "${ENCODE_CMD[@]}" >"${ENC_LOG}" 2>&1 &
else
    "${ENCODE_CMD[@]}" >"${ENC_LOG}" 2>&1 &
fi
PIDS+=($!)


declare -a PREFILL_ENV=("CUDA_VISIBLE_DEVICES=$GPU_P" "UCX_NET_DEVICES=all")
if [[ "$PREFILL_SHARES_DECODE_GPU" == "1" ]]; then
    PREFILL_ENV+=(
        "CUDA_MPS_ACTIVE_THREAD_PERCENTAGE=$NON_DECODE_MPS_ACTIVE_THREAD_PERCENTAGE"
        "CUDA_DEVICE_MAX_CONNECTIONS=$NON_DECODE_CUDA_DEVICE_MAX_CONNECTIONS"
    )
...
if [[ "$PREFILL_SHARES_DECODE_GPU" == "1" ]] && command -v nice >/dev/null 2>&1; then
    nice -n "$NON_DECODE_NICE" "${PREFILL_CMD[@]}" >"${P_LOG}" 2>&1 &
else
    "${PREFILL_CMD[@]}" >"${P_LOG}" 2>&1 &
fi
PIDS+=($!)
...
for idx in "${!DECODE_GPUS[@]}"; do
    ...
    decode_shares_other="${DECODE_SHARES_OTHER[$idx]}"

    declare -a DECODE_ENV=("CUDA_VISIBLE_DEVICES=$decode_gpu" "UCX_NET_DEVICES=all")
    if [[ "$decode_shares_other" == "1" ]]; then
        DECODE_ENV+=(
            "CUDA_MPS_ACTIVE_THREAD_PERCENTAGE=$DECODE_MPS_ACTIVE_THREAD_PERCENTAGE"
            "CUDA_DEVICE_MAX_CONNECTIONS=$DECODE_CUDA_DEVICE_MAX_CONNECTIONS"
        )
    fi
    if [[ "$KV_CONNECTOR_NAME" == "NixlConnector" ]]; then
